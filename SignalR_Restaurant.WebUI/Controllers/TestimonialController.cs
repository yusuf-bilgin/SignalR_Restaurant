...
namespace SignalR_Restaurant.WebUI.Controllers
{
    public class TestimonialController : Controller
    {
        ...
        public async Task<IActionResult> Index()
        {
            ...
            var responseMessage = await client.GetAsync("https://localhost:44369/api/Testimonial"); //api'nin çalıştığı port (yanlışlıkla UI'ın çalıştığı portu yazma)
        ...
        public async Task<IActionResult> CreateTestimonial(CreateTestimonialDto createTestimonialDto)
        {
            ...
            var responseMessage = await client.PostAsync("https://localhost:44369/api/Testimonial", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
        ...
        public async Task<IActionResult> UpdateTestimonial(UpdateTestimonialDto updateTestimonialDto)
        {
            ...
            StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json");
            var responseMessage = await client.PutAsync("https://localhost:44369/api/Testimonial/", stringContent);
            if (responseMessage.IsSuccessStatusCode)
            {
                return RedirectToAction("Index");
            }
